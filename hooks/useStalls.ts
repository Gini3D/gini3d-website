...
function parseStallEvent(event: {
  ...
  try {
    ...
    const shipping: StallShippingZone[] = (content.shipping || []).map(
      (zone: { id: string; name?: string; cost: number; regions?: string[] }) => ({
        id: zone.id,
        name: zone.name,
...
export function useStalls(sellerPubkeys: string[]) {
  ...
  const getSellerShipping = useCallback(
    (pubkey: string): StallShippingZone[] => {
  ...
  const getSellerCurrency = useCallback(
